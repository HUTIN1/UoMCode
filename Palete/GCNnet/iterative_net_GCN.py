...
class GCNNet(pl.LightningModule):
    def __init__(self, lr=1e-4,batch_size=1,num_classes=2,in_features = 3,model_iterative = []) -> None:
        ...
        self.model_iterative = model_iterative

    def iterative_model(self,batch):
        for model in self.model_iterative:
            out = model(batch)
            pred = out.argmax(dim = -1, keepdim = True).squeeze()
            where = torch.argwhere(pred)
            batch.x = batch.x[where].squeeze()
            batch.x = 1
