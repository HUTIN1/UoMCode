...
from utils_GCN import  MatrixScale, MatrixTranspose, GetUnitSurf
...
class UnitSurfTransform:
    ...
    def __init__(self, random_rotation=False,scale = 1):
    ...
    def __call__(self, surf):
        ...
        surf, mean, scale = GetUnitSurf(surf)
        # if self.random_rotation:
        #     surf, _a, _v = utils_GCN.RandomRotation(surf)
        kwargs= {'mean':mean,'scale':scale}
        matrix_transpose  = MatrixTranspose(-mean)
        matrix_scale = MatrixScale(scale)
        matrix = np.matmul(matrix_scale,matrix_transpose)
        return surf, matrix
...
#     def __call__(self,surf):
#         kwargs={}
#         surf , angle, vector = utils_GCN.RandomRotation(surf)
#         kwargs['angle']=angle
...
#         matrix_transpose  = MatrixTranspose(-mean)
#         matrix_scale = MatrixScale(scale)
#         matrix = np.matmul(matrix_transpose,matrix_scale)
...
class MyCompose:
    ...
    def __call__(self,data):
        ...
        for transform in self.transform:
            data, matrix_out = transform(data)
            matrix = np.matmul(matrix_out,matrix)
        return data, matrix